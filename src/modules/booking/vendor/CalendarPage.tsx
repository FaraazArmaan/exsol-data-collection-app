...
function weekOf(dateStr: string): string[] {
  ...
  return Array.from({ length: 7 }, (_, i) => { const x = new Date(start); x.setDate(start.getDate() + i); return localDate(x.toISOString()); });
}

// Greedy lane-packing so concurrent bookings in one day column sit side by side.
function packDay(events: VendorBooking[]): Array<{ b: VendorBooking; lane: number; lanes: number }> {
  const sorted = [...events].sort((a, b) => a.start_at.localeCompare(b.start_at));
  const laneEnds: number[] = [];
  const rows = sorted.map((b) => {
    const s = localMin(b.start_at), e = localMin(b.end_at);
...
export default function CalendarPage({ slug, perms }: Props) {
  ...
  function reload() {
    ...
    bookingApi.list(new URLSearchParams(range).toString()).then((r) => setBookings(r.bookings)).catch(() => setBookings([]));
  }
  useEffect(() => {
  ...
  let gStart = 9 * 60, gEnd = 18 * 60;
  if (settings) {
    const wins = view === 'week'
      ? Object.values(settings.weekly_schedule).flat()
      : (settings.weekly_schedule[WEEKDAY[new Date(`${date}T12:00:00`).getDay()]!] ?? []);
    if (wins.length) { gStart = Math.min(...wins.map((w) => toMin(w.open))); gEnd = Math.max(...wins.map((w) => toMin(w.close))); }
  }
  for (const b of bookings ?? []) { gStart = Math.min(gStart, localMin(b.start_at)); gEnd = Math.max(gEnd, localMin(b.end_at)); }
  gStart = Math.floor(gStart / 60) * 60; gEnd = Math.ceil(gEnd / 60) * 60;
  ...
  return (
    <div className="page booking-vendor">
      ...
      {!bookings || !settings ? <div className="muted">Loading…</div>
        : view === 'week' ? (
          <div className="booking-grid" style={{ gridTemplateColumns: `56px repeat(7, minmax(120px, 1fr))` }}>
            <div className="booking-grid-corner" />
            {days.map((d) => {
              const dd = new Date(`${d}T12:00:00`);
              return (
                <button key={d} className={`booking-grid-colhead booking-week-head${d === date ? ' is-today' : ''}`} onClick={() => { setDate(d); setView('day'); }}>
            ...
            <div className="booking-grid-gutter" style={{ height: gridH }}>
              {hours.map((h) => <span key={h} className="booking-grid-hourlabel" style={{ top: (h - gStart) * PX_PER_MIN }}>{pad(Math.floor(h / 60))}:00</span>)}
            </div>
            {days.map((d) => {
              const packed = packDay((bookings ?? []).filter((b) => localDate(b.start_at) === d));
              return (
                <div key={d} className="booking-grid-col" style={{ height: gridH }}
                  onClick={(e) => { if (canCreate) { const m = snapAt(e); setCreating({ defaultStart: `${d}T${pad(Math.floor(m / 60))}:${pad(m % 60)}` }); } }}>
                  {hours.map((h) => <div key={h} className="booking-grid-line" style={{ top: (h - gStart) * PX_PER_MIN }} />)}
                  {packed.map(({ b, lane, lanes }) => (
                    <button key={b.id} className={`booking-grid-block block-${b.status}`} style={posStyle(b, lane, lanes)}
                      ...
                      {lanes === 1 ? <span className="booking-week-res">{resName(b.resource_id)}</span> : null}
                    </button>
                  ))}
                </div>
              );
          ...
          <div className="booking-grid" style={{ gridTemplateColumns: `56px repeat(${resources.length}, minmax(140px, 1fr))` }}>
            ...
            {resources.map((r) => (
              <div key={r.id} className="booking-grid-col" style={{ height: gridH }}
                onClick={(e) => { if (canCreate) { const m = snapAt(e); setCreating({ resourceId: r.id, defaultStart: `${date}T${pad(Math.floor(m / 60))}:${pad(m % 60)}` }); } }}>
                {hours.map((h) => <div key={h} className="booking-grid-line" style={{ top: (h - gStart) * PX_PER_MIN }} />)}
                {byResource(r.id).map((b) => (
                  <button key={b.id} className={`booking-grid-block block-${b.status}`} style={posStyle(b)}
                    onClick={(e) => { e.stopPropagation(); setOpenId(b.id); }}>
