...
export default function CalendarPage({ slug, perms }: Props) {
  ...
  // Day-grid time window: the weekday's open window, expanded to cover off-grid bookings.
  ...
  if (view === 'day') for (const b of bookings ?? []) { dayStart = Math.min(dayStart, localMin(b.start_at)); dayEnd = Math.max(dayEnd, localMin(b.end_at)); }
  ...
  return (
    <div className="page booking-vendor">
      ...
      {!bookings || !settings ? <div className="muted">Loading…</div>
        : view === 'week' ? (
          <div className="booking-week" style={{ gridTemplateColumns: 'repeat(7, minmax(120px, 1fr))' }}>
            {days.map((d) => {
              const dd = new Date(`${d}T12:00:00`);
              const dayBookings = (bookings ?? []).filter((b) => localDate(b.start_at) === d).sort((a, b) => a.start_at.localeCompare(b.start_at));
              return (
                <div key={d} className={`booking-week-col${d === date ? ' is-today' : ''}`}>
                  <button className="booking-week-colhead" onClick={() => { setDate(d); setView('day'); }}>
                    <span>{dd.toLocaleDateString([], { weekday: 'short' })}</span>
                    <strong>{dd.getDate()}</strong>
                  </button>
                  <div className="booking-week-list">
                    {dayBookings.map((b) => (
                      <button key={b.id} className={`booking-week-card block-${b.status}`} onClick={() => setOpenId(b.id)}>
                    ...
                    {dayBookings.length === 0 ? <span className="booking-week-empty">—</span> : null}
                  </div>
                </div>
              );
            })}
