...
function period(iso: string) {
  const h = new Date(iso).getHours();
  return h < 12 ? 'Morning' : h < 17 ? 'Afternoon' : 'Evening';
}

export function SlotPicker({ slug, services, onPick, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    bookingPublicApi
      .resources(slug)
      .then((r) => {
        if (!cancel) setResources(r.resources);
      })
      .catch(() => {});
    return () => {
      cancel = true;
  ...
  return (
    <div className="booking-sf-step-panel">
      <button className="booking-sf-back" onClick={onBack}>
        ← Services
      </button>
      <div className="booking-sf-chosen">
        ...
        <span className="booking-service-meta">
          ...
          {formatRupees(services.reduce((total, service) => total + service.price_cents, 0))}
        </span>
      </div>
      ...
      <div className="booking-datestrip">
        <button
          className="booking-datestrip-nav"
        ...
        <div className="booking-datestrip-days">
          {week.map((d) => {
            ...
            return (
              <button
                key={d}
                className={`booking-daychip${d === date ? ' is-active' : ''}`}
                disabled={disabled}
                onClick={() => setDate(d)}
              >
        ...
        <button
          ...
          disabled={isoDatePlus((weekOffset + 1) * 7) > isoDatePlus(MAX_DAYS)}
      ...
      {resources.length > 0 && (
        <>
          ...
          <div className="booking-chips">
            <button
              className={`booking-chip-btn${resourceId === 'any' ? ' is-active' : ''}`}
            ...
            {resources.map((r) => (
              <button
                key={r.id}
                className={`booking-chip-btn${resourceId === r.id ? ' is-active' : ''}`}
      ...
      {slots === null ? (
        <div className="booking-sf-empty">Loading times…</div>
      ) : slots.length === 0 ? (
        <p className="booking-sf-empty">No times available on this day. Try another date.</p>
      ) : (
