// Procurement authorization. Mirrors _inventory-authz.requireInventory.
//
// Two layers, in order (Iron Rule 2):
//   1. Enable-gate — the procurement MODULE must be reachable from an enabled
//      product for this Client (412 procurement_module_not_enabled otherwise).
//   2. Permission — the caller must hold the explicit procurement.products.<verb>
//      key, EXCEPT L1 (Primary/Owner), who is treated as all-on. For L1 we return
//      the FULL procurement.* set in ctx.perms so downstream verb checks treat the
...
export interface ProcurementAuthCtx {
  userNodeId: string;
  clientId: string;
...
export async function requireProcurement(
...
): Promise<{ ok: true; ctx: ProcurementAuthCtx } | { ok: false; res: Response }> {
  ...
  // Module gate: is 'procurement' brought in by any enabled product for this client?
  ...
  if (!modules.has('procurement')) {
    return { ok: false, res: jsonError(412, 'procurement_module_not_enabled') };
  }
  ...
  if (levelNumber === 1) {
    return {
      ...
      ctx: {
        ...
        perms: new Set(ALL_PROCUREMENT_PERMS),
      },
    };
  }
