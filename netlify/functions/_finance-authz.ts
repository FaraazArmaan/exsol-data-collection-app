// Finance authorization. Mirrors _booking-authz.requireBooking / _pos-authz.
//
// Two layers, in order:
//   1. Enable-gate — the finance MODULE must be reachable from an enabled
//      product for this Client (412 finance_module_not_enabled otherwise).
//   2. Permission — the caller must hold the explicit finance.<bucket>.<verb>
//      key, EXCEPT L1 (Primary/Owner), who is treated as all-on. A strict
//      matrix-only check would blank the Owner's UI (they're privileged by
//      level, not by explicit grants) — this is the codebase's iron rule.
//
// For L1 we return the FULL finance.* set in ctx.perms so downstream verb
// checks treat the Owner as fully privileged, not merely able to pass `required`.
...
export async function requireFinance(
...
): Promise<{ ok: true; ctx: FinanceAuthCtx } | { ok: false; res: Response }> {
  ...
  // Module gate: is 'finance' brought in by any enabled product for this client?
  ...
  if (!modules.has('finance')) {
    return { ok: false, res: jsonError(412, 'finance_module_not_enabled') };
  }

  // L1 (Primary/Owner) is all-on, consistent with every other gate. The
  // enable-gate above is the product-level check; the Owner needs no
  // surface-level grant. Hand back the FULL finance.* set.
  if (levelNumber === 1) {
    return {
      ok: true,
      ctx: {
        ...
        perms: new Set(ALL_FINANCE_PERMS),
      },
    };
  }
