// Warehouse authorization. Mirrors _inventory-authz.requireInventory.
//
// Two layers, in order (Iron Rule 2):
//   1. Enable-gate — the warehouse MODULE must be reachable from an enabled
//      product for this Client (412 warehouse_module_not_enabled otherwise).
//   2. Permission — the caller must hold the explicit warehouse.<bucket>.<verb>
//      key, EXCEPT L1 (Primary/Owner), who is treated as all-on. For L1 we return
//      the FULL warehouse.* set in ctx.perms so downstream verb checks treat the
...
export interface WarehouseAuthCtx {
  userNodeId: string;
  clientId: string;
...
export async function requireWarehouse(
...
): Promise<{ ok: true; ctx: WarehouseAuthCtx } | { ok: false; res: Response }> {
  ...
  // Module gate: is 'warehouse' brought in by any enabled product for this client?
  ...
  if (!modules.has('warehouse')) {
    return { ok: false, res: jsonError(412, 'warehouse_module_not_enabled') };
  }
  ...
  if (levelNumber === 1) {
    return {
      ...
      ctx: {
        ...
        perms: new Set(ALL_WAREHOUSE_PERMS),
      },
    };
  }
