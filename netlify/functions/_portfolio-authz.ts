// Brand Portfolio Site authorization. Mirrors _email-authz / _booking-authz.
//
// Two layers, in order:
//   1. Enable-gate — the portfolio MODULE must be reachable from an enabled
//      product for this Client (412 portfolio_module_not_enabled otherwise).
//   2. Permission — the caller must hold the explicit portfolio.<bucket>.<verb>
...
export interface PortfolioAuthCtx {
  userNodeId: string;
  clientId: string;
...
export async function requirePortfolio(
...
): Promise<{ ok: true; ctx: PortfolioAuthCtx } | { ok: false; res: Response }> {
  ...
  // Module gate: is 'portfolio' brought in by any enabled product for this client?
  ...
  if (!modules.has('portfolio')) {
    return { ok: false, res: jsonError(412, 'portfolio_module_not_enabled') };
  }
  ...
  if (levelNumber === 1) {
    return {
      ...
      ctx: {
        ...
        perms: new Set(ALL_PORTFOLIO_PERMS),
      },
    };
  }
