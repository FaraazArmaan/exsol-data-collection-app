...
import { sanitizeCampaignHtml } from './_shared/html-sanitize';
...
export default async function handler(req: Request): Promise<Response> {
  ...
  // Strip stored-XSS vectors (script/on*/javascript:) before persisting — the body
  // is rendered to other staff and emailed to recipients. See _shared/html-sanitize.
  const bodyHtml = sanitizeCampaignHtml(b.body_html);
  const sql = db();
  const rows = (await sql`
    ...
    VALUES (${a.ctx.clientId}::uuid, ${b.name.trim()}, ${b.subject.trim()}, ${bodyHtml}, ${audience}, ${channel}, ${isAb}, ${subjectB}, ${abSplit}, 'draft', ${a.ctx.userNodeId}::uuid)
