// Orders authorization. Mirrors _inventory-authz.requireInventory / _finance-authz.requireFinance.
//
// Two layers, in order (Iron Rule 2):
//   1. Enable-gate — the orders MODULE must be reachable from an enabled
//      product for this Client (412 orders_module_not_enabled otherwise).
//   2. Permission — the caller must hold the explicit orders.business.<verb>
//      key, EXCEPT L1 (Primary/Owner), who is treated as all-on. For L1 we return
//      the FULL orders.* set in ctx.perms so downstream verb checks treat the
//      Owner as fully privileged, not merely able to pass `required`.
...
export interface OrdersAuthCtx {
  userNodeId: string;
  clientId: string;
  levelNumber: number;
...
export async function requireOrders(
  req: Request,
  required: readonly string[],
): Promise<{ ok: true; ctx: OrdersAuthCtx } | { ok: false; res: Response }> {
  ...
  // Module gate: is 'orders' brought in by any enabled product for this client?
  ...
  if (!modules.has('orders')) {
    return { ok: false, res: jsonError(412, 'orders_module_not_enabled') };
  }
  ...
  if (levelNumber === 1) {
    return {
      ...
      ctx: {
        ...
        levelNumber: 1,
        perms: new Set(ALL_ORDERS_PERMS),
      },
    };
  }
  ...
  return { ok: true, ctx: { userNodeId: credential.user_node_id, clientId: claims.client_id, levelNumber, perms } };
}
