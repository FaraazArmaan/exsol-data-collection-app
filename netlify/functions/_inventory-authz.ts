// Inventory authorization. Mirrors _booking-authz.requireBooking / _pos-authz.requirePos.
//
// Two layers, in order (Iron Rule 2):
//   1. Enable-gate — the inventory MODULE must be reachable from an enabled
//      product for this Client (412 inventory_module_not_enabled otherwise).
//   2. Permission — the caller must hold the explicit inventory.products.<verb>
//      key, EXCEPT L1 (Primary/Owner), who is treated as all-on. For L1 we return
//      the FULL inventory.* set in ctx.perms so downstream verb checks treat the
...
export interface InventoryAuthCtx {
  userNodeId: string;
  clientId: string;
...
export async function requireInventory(
...
): Promise<{ ok: true; ctx: InventoryAuthCtx } | { ok: false; res: Response }> {
  ...
  // Module gate: is 'inventory' brought in by any enabled product for this client?
  ...
  if (!modules.has('inventory')) {
    return { ok: false, res: jsonError(412, 'inventory_module_not_enabled') };
  }
  ...
  if (levelNumber === 1) {
    return {
      ...
      ctx: {
        ...
        perms: new Set(ALL_INVENTORY_PERMS),
      },
    };
  }
