...
beforeAll(async () => {
  ...
  await setBookingSettings(clientId, { mon: [{ open: '09:00', close: '18:00' }] }, { slot_interval_min: 30 });
  payService = await makeService(clientId, { name: 'Cut', duration_min: 60, payment_mode: 'pay_at_venue', eligible_resource_ids: [resId] });
  depositService = await makeService(clientId, { name: 'Color', duration_min: 60, price_cents: 50000, payment_mode: 'deposit', deposit_cents: 10000, eligible_resource_ids: [resId] });
});

const body = (service: string, start: string, i = 0) => ({
  service_id: service, resource_id: 'any', start,
...
describe('POST public create', () => {
  it('pay_at_venue → 201 confirmed + manage_token', async () => {
    const r = await create(publicRequest(slug, 'POST', '/create', body(payService, '2026-08-17T05:00:00.000Z')));
  ...
  it('named resource booked twice on an overlapping slot → 409 slot_taken (gist 23P01)', async () => {
    ...
    const named = (i: number) => ({
      service_id: payService, resource_id: resId, start,
  ...
  it('"any" with the only resource busy → 409 no_resource_available (pre-check)', async () => {
    ...
    expect((await create(publicRequest(slug, 'POST', '/create', body(payService, start, 8)))).status).toBe(201);
  ...
  it('deposit service → 201 pending + payment_intent', async () => {
    const r = await create(publicRequest(slug, 'POST', '/create', body(depositService, '2026-08-17T08:00:00.000Z', 3)));
    expect(r.status).toBe(201);
    const j = await r.json();
